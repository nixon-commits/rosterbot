...
- **EventBridge rules** — 18 schedule rules (1:1 port of the old GitHub Actions crons, UTC, plus `ProjectionSite`, `Archive`, `TeamValues`, `Shadow`, `VersionCheck`, `ProjectionReports`, `FootballValues`, `FootballTrades`, and `LineupMatchup` — the once-daily `optimize --matchup` future-day pass split off the hourly `Lineup` job, rosterbot-cem), plus two notification rules (`BuildNotifyRule`, `TaskFailRule`) that both target the `OpsNotify` Lambda
...
- **SSM Parameter Store** (`/rosterbot/*`) — secrets as SecureString, injected as task env; plus plain-String plumbing values that are neither secret nor injected (`/rosterbot/DASHBOARD_CF_DIST_ID`, and `/rosterbot/SITE_CERT_ARN`, written by `InfraCertStack` on every certificate change and read by `InfraStack`'s two CloudFront distributions as the CloudFormation dynamic reference `{{resolve:ssm:/rosterbot/SITE_CERT_ARN}}` — the whole cert handoff between the two regions, see `docs/adr/0004-cert-arn-handoff-via-ssm-parameter.md`).
- **CodeBuild** — on push to `main`, builds + pushes the image to ECR, then runs **`cdk deploy --all -c enableBuild=true`** so infrastructure changes (schedules, task defs, IAM, Lambda) ship on merge — not just the image — before publishing the dashboard (reading the deploy's own outputs, see above) and launching the `projection-site` task (`ecs:RunTask` via `taskDef.GrantRun`, reusing `TaskSg` + public subnets) so it re-renders immediately with the new image. Before the `cdk deploy` step existed, a PR touching `infra/` merged green but its infra change sat undeployed until someone ran `cdk deploy` by hand (this is what left the `Archive` schedule inert for ~25h); deploying first also means a broken infra change now fails the build before the dashboard/projection-site steps run, rather than shipping around it. The `--all` is mandatory too, and for a blunter reason: the app has held more than one stack since `InfraCertStack` (the us-east-1 rosterbot.dev certificate), and cdk refuses a bare `cdk deploy` on a multi-stack app — it exits 1, so every push to `main` fails at this step until the flag is there. Nothing local catches that; `go build`/`go vet`/`go test`/`make build-modules`/`make check-pins` are all green and a developer running `cdk deploy --all` by hand sees it work, so it surfaces only in CodeBuild after merge (`infra/buildspec_test.go` now pins it). The `enableBuild=true` is mandatory — without it the deploy would delete the CodeBuild project running it. cdk works through the bootstrap roles, so the build role is granted only `sts:AssumeRole` on `cdk-hnb659fds-*`. The build host ships Go 1.20 + no cdk CLI, so the `install` phase adds Go 1.25 (`GOTOOLCHAIN=auto` upgrades further if a go.mod requires it) and the pinned cdk CLI. Gated by `enableBuild`. Four latency choices, each pinned by a test in `infra/` (measured baseline: build 290, 2026-08-29, 6:20 total): the project carries an **S3 cache** (`BuildCache` bucket, 30-day expiry) holding `GOCACHE`/`GOMODCACHE` across builds, without which `cdk synth` recompiles the aws-cdk-go bindings + both lambda bundles cold every push (130s of that baseline; warm synth measured 31s on build 295); the deploy runs **`--method=direct`**, skipping the ~26s/stack changeset nothing in CI reads; the GitHub source clones at **depth 1**, since nothing in the build reads git history; and **Node is installed by an install-phase command from a cached dist tarball, never via `runtime-versions`** — CodeBuild installs runtime-versions during DOWNLOAD_SOURCE *before* the S3 cache restore (measured on build 295), so a `nodejs:` pin re-downloads Node from nodejs.org on every fresh host (~33–45s) and no cache path can help it. Because this fires on `main` only, **a `cdk deploy` run from a feature branch is undone by the next merge to `main`** — see the warning under *Common operations* before deploying from a branch.
- **BuildCache growth watch (rosterbot-b6bg)** — the `BuildCache` S3 object is one opaque tarball packing `GOCACHE` + `GOMODCACHE` + `NODE_DIST_CACHE`; CodeBuild's own "Cache uploaded successfully (size: ...)" log line only reports that combined total, which grew unmitigated from 466.49 MiB (build 294, 2026-08-29) to 1.81 GiB (build 312, 2026-09-07). `buildspec.yml`'s `install` phase now prints an unconditional `cache sizes: GOCACHE=<n> MiB GOMODCACHE=<n> MiB NODE_DIST_CACHE=<n> MiB total=<n> MiB` line every build (from `du -sm` on each pinned dir, in the CloudWatch build log) — that per-directory split is the measurement the combined total can never provide, so it prints whether or not the gate below fires. Immediately after, still in `install` (before this build's own `go version` sanity check, so nothing has written into a fresh `GOCACHE` yet), a size gate runs `go clean -cache` only when `$GOCACHE` itself exceeds **768 MiB** — a stated prior, not a measurement: comfortably above a single warm build's `GOCACHE`, comfortably below the 1.81 GiB already reached. The gate targets `GOCACHE` specifically, never the total and never `GOMODCACHE` (module downloads are content-addressed; `go clean -modcache` would throw away the expensive part), so a `GOMODCACHE`-driven growth pattern would need a separate, not-yet-built gate — watch the `cache sizes:` line on subsequent builds to see which directory is actually driving growth. Placement in `install`, not `post_build`, is deliberate: pruning after the build would upload an empty `GOCACHE`, so the very next build pays a cold recompile **and** re-uploads; pruning here means a triggered build pays the cold recompile once and still uploads a useful cache. Watch for the gate firing on nearly every build rather than occasionally — that means the threshold is undersized and is paying the ~130s cold `aws-cdk-go` recompile (build 290) more often than the upload tax it's meant to save.
- **Two builds racing on `cdk deploy`** — CodeBuild does not serialize builds for this project (`ConcurrentBuildLimit` stays `-1`, `infra/infra.go`, rosterbot-7p1i/rosterbot-ill), because CodeBuild's limit *throttles* an excess build rather than queueing it — a build inside the limit is silently **discarded**: no build record, no log, no Pushover. That was tried and reverted the same day after it dropped a live outage fix. What remains real is two pushes landing minutes apart both reaching `cdk deploy` on `InfraStack`: the loser's `UpdateStack` fails while the stack is still `..._IN_PROGRESS` from the winner, a lock that clears on its own within a few minutes. `buildspec.yml`'s `post_build` now wraps the deploy in `infra/cdk_deploy_retry.sh` (rosterbot-udd), which retries **only** on that specific, self-clearing signature (`UPDATE_IN_PROGRESS`, `is currently being updated`, `OperationInProgressException`, etc. — see the script for the full list), bounded attempts with backoff, and still exits non-zero on any other failure or once attempts are exhausted — so a genuine deploy error still fails the build loudly rather than trading a dropped build for a masked one. Separately, and regardless of the race: a missing or incomplete `cdk-outputs.json` after the deploy step now fails the build with an explicit message naming the missing file or key, rather than surfacing three steps later as a bare Python traceback out of the dashboard-publish step's `DashboardBucketName`/`DashboardCdnId` lookups.
...
- **us-east-1 is bootstrapped** (`CDKToolkit`, created 2026-08-18). The stack runs in us-west-1,
  ...
  `InfraCertStack` deploy fails on `BootstrapVersionValidation`, and `InfraStack` — which
  `cdk deploy --all` orders after it through an explicit stack dependency — is never reached.
  The certificate's ARN crosses into `InfraStack` through the SSM parameter
  `/rosterbot/SITE_CERT_ARN`, deliberately NOT through a CDK cross-region reference: CDK's
  built-in exporter is the seat of a known, upstream-rooted bug (rosterbot-klbh) in which a
  certificate replacement left its generated export parameter permanently stale, which read
  as a certificate problem and was a plumbing one. `docs/adr/0004-cert-arn-handoff-via-ssm-parameter.md`
  has the mechanism, the producer/consumer pair that replaced it, and the one property of
  dynamic references to know before a cert change (CloudFormation re-resolves the parameter
  only on an `InfraStack` update, and `cdk deploy` skips a stack whose template is unchanged).
