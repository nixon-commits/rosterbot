...
**Playoff Round**:
One round of the post-season bracket — a Weekly Period like any other (its own number, dates and Fantrax-side GS limits), flagged `ScoringPeriod.Playoff`, in which only the teams paired in the round have a scoring matchup. The standings SCHEDULE view stops at the regular season, so the rounds come from `getStandings view=PLAYOFFS` (`GetPlayoffBracket`, merged in by `GetScoringPeriodsAndTeams` and `allMatchups`). Measured 2026-09-14: 2026 ran three rounds over periods 23-25 (Sep 7-27) with two byes in Round 1, and until rosterbot-0lyz every season-boundary guard took the regular-season end for the end of the season. A bye is a Playoff Round the team is in but not paired in; it is reported on `MatchupEntry.Bye`, never as a matchup. Fantrax intermittently renders an in-progress round as seed placeholders (measured 2026-09-14, three probes over two hours: named, seeds, named), so an alive team can be named nowhere in its round; `playoffMatchups` gives every team that advanced out of the previous round a **pending** row (own id, no opponent) in an undrawn round, which is what keeps its Matchup Week — and its lineup — alive through that state. `PlayoffStatusFor` reads a team's standing (alive / bye / eliminated in round N with the score line / never seeded) off the bracket for wording only; the decision to stop is always the Matchup Week's.

**Scoring Day**:
A calendar date on which one team's lineup scored for something — the question the lineup stop and the lineup-gap grading both ask, answered once by `fantrax.ClassifyScoringDay` (rosterbot-zg1r). Three verdicts: a date inside a regular-season Weekly Period is scoring by construction (every team has a matchup every week, so a missing row there is a Fantrax fault the window resolvers keep loud, never a reason to drop a day); a date inside a Playoff Round is scoring only if the team has a Matchup Week containing it (a bye, an elimination or a never-seeded team is the ordinary "no"); a date no Weekly Period contains is outside the season. A lookup failure is an error, never a verdict, and each caller decides what an unknown costs it: `lineuprun.playoffIdle` optimizes anyway, `grade` withholds the run's gap rows (the next run's floor re-grades them), `backtest` grades the lineup unfiltered and says so. Projection grading is deliberately NOT gated on it — a projection is judged against real MLB actuals, which keep arriving whether or not the fantasy lineup counts — and after an elimination `shadow` writes no capture anyway, so the accuracy series simply ends.
