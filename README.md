...
# and champion, the season awards (which no longer sit on every week page), full-season
# lineup efficiency, the trades of the year (HKB-valued) and the season league leaders.
...
Required for the dynasty football (Sleeper) commands — `football-values`, `football-trades` — and *only* those; no `FANTRAX_*` var is needed to run them: `SLEEPER_LEAGUE_ID`, plus `SLEEPER_USER_ID` for `football-trades`.
