...
if command -v bd >/dev/null 2>&1; then
  ...
  echo "      fetch beads_1.0.4_<os>_<arch>.tar.gz from"
  ...
  echo "      extract bd onto PATH (e.g. /usr/local/bin). Prefer that over"
  echo "      'go install', which cold-builds bd's Dolt-linked ~180 MB binary and"
  echo "      needs a C++ toolchain plus ICU headers."
