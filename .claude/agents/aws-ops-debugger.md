...
1. **Check the run ledger first — it's the fastest path.** The entrypoint writes one JSON per run to `runs/<invertedTs>-<taskId>.json` in the state bucket (newest-first key order); FAILED entries carry the exit code and a **log tail**. Two ways in:
   ...
   - Or straight from S3: `aws s3 ls s3://<StateBucketName>/runs/ | head`, then `aws s3 cp s3://<StateBucketName>/runs/<key> -` and read `status`, `exitCode`, `logTail`, `runTrigger` (`schedule` vs `manual`).
