...
**Blended scoring** — wraps the configured FanGraphs source with recent Fantrax stats (last 10 scoring periods). Falls back to 100% base projection when no recent data. Recent stats are fetched in parallel via `errgroup` in `fantrax/recent_stats.go`. Weights are **dynamic** (not fixed) and shift toward recent stats as a player accumulates a stable sample.
