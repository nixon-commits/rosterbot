...
**Multi-system capture (`shadow`)** — `cmd/shadow.go` drives the full `optimize` pipeline (matchup adjustment included) once per RoS system in dry-run, capturing every system's projection side-by-side for the model-comparison report — no lineup applied, no notification. It reuses `runOptimize` verbatim (zero pipeline duplication): it sets `captureSystemRoot` (a package var in `optimize.go`) to redirect `writeProjectionSnapshot` to `.backtest/snapshots-systems/system=<sys>/<date>.json` (instead of the flat `.backtest/snapshots/`), forces dry-run + snapshot-on, and loops `projectionSystem` over `shadowSystems` (`steamer-ros`, `depthcharts-ros`, `thebatx-ros`, `atc-ros`). `grade` then loops the same `shadowSystems`, calling `RunProjectionAnalysis(days, systemSnapshotDir(...))` per system (actuals fetched once) and writing each system's rows under its `system=` partition. The hourly `optimize`'s flat snapshot is untouched (it still feeds the local `backtest` command). The EventBridge `Shadow` schedule runs **23:40 UTC** — late ET evening but still the **same ET calendar day as the captured date**, so the snapshot's `generated_at` passes the `sameETDate` stale-guard and is graded by the next 13:30 UTC `grade` run. `.backtest/snapshots-systems/` rides the existing recursive `.backtest/` S3 sync (`del=false`), so the shadow task's `sync-up` and grade task's `sync-down` hand it across Fargate tasks with no entrypoint change.
