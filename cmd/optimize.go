...
func runOptimize(cmd *cobra.Command, args []string) error {
	...
	if periodErr != nil || currentPeriod <= 1 {
		...
		recentStats, err := windowedHitterRecent(ft, cfg.TeamID, today, seasonStart)
		if err != nil {
...
func windowedHitterRecent(ft *fantrax.Client, teamID string, today, seasonStart time.Time) (map[string]fantrax.RecentStat, error) {
	...
	days, err := ft.DailyFantasyPoints(teamID, start, yesterday, seasonStart, cacheDir, cacheTTL(30*24*time.Hour))
	if err != nil {
		return nil, err
	}
