...
func toWireResult(vs []Violation, period string, gsMax, gsMin int) jobwire.GSCheckResult {
	out := jobwire.GSCheckResult{Period: period}
	for _, v := range vs {
		o := jobwire.GSViolationOut{Team: v.TeamName, Used: v.GSUsed}
		switch v.Kind {
		case ViolationMax:
			o.Kind = "over"
			o.Limit = gsMax
