...
func TestPitcherBlendWeights_SP_Progression(t *testing.T) {
	tests := []struct {
		...
		{4, 0.79, 0.02},  // early: 4/(4+15) ≈ 0.21 season → ~79% base
	...
	for _, tt := range tests {
		...
		if math.Abs(sw-tt.expectBase) > tt.tolerance {
			t.Errorf("SP GP=%d: expected base ~%.2f, got %.4f", tt.gp, tt.expectBase, sw)
		}
...
func TestPitcherBlendWeights_RP_Progression(t *testing.T) {
	tests := []struct {
		...
		{4, 0.86, 0.02},  // early: 4/(4+25) ≈ 0.14 season → ~86% base
	...
	for _, tt := range tests {
		...
		if math.Abs(sw-tt.expectBase) > tt.tolerance {
			t.Errorf("RP GP=%d: expected base ~%.2f, got %.4f", tt.gp, tt.expectBase, sw)
		}
		if math.Abs((sw+rw)-1.0) > 1e-9 {
...
func TestPitcherBlendWeights_BaseFloor(t *testing.T) {
	for gp := 0; gp <= 100; gp++ {
		for _, isSP := range []bool{true, false} {
			sw, _ := PitcherBlendWeightsForDisplay(gp, isSP)
			if sw < pitcherBaseFloor-1e-9 {
				role := "RP"
				if isSP {
					role = "SP"
				}
				t.Errorf("%s GP=%d: base weight %.4f below floor %.2f", role, gp, sw, pitcherBaseFloor)
			}
		}
	}
}
